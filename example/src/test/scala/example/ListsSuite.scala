 ...
 */
 class ListsSuite {
  ...
  @Test def `max of negative numbers`: Unit = {
    assert(max(List(-2, -4, -1)) == -1)
  }

  @Test def `max of some equal numbers (10pts)`: Unit = {
    assert(max(List(3, 3, 2)) == 3)
  }
  
  def listNotEmpty(x: List[Int]): Int = {
    if (x.isEmpty) throw new IllegalArgumentException("Empty list has no max")
    else max(x)
  }

 @Test def `non empty in Max`: Unit = {
 try {
   listNotEmpty(List())
   Assert.fail("No exception has been thrown")
   } catch {
